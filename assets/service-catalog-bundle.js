import { s as styled, x as getColor, N as Avatar, j as jsxRuntimeExports, aA as SvgShapesFill, e as sanitizeHtml, ai as Grid, au as Skeleton, u as useTranslation, aB as LG, aC as MD, G as Button, aD as IconButton, T as Tooltip, aE as SvgXStroke, b as Field, M as MediaInput, a3 as SvgSearchStroke, Q as Dots, r as reactExports, n as notify, l as debounce, av as CursorPagination, Z as initI18next, _ as loadTranslations, a0 as reactDomExports, a1 as ThemeProviders, a2 as createTheme, ay as ErrorBoundary, aF as linkifyStr, aG as XXXL, aH as SvgChevronUpFill, aI as SvgChevronDownFill, O as Option, S as Span, A as Anchor, H as addFlashNotification } from 'shared';
...
const useAssetDataFetchers = (assetOptionId, assetTypeOptionId) => {
    const fetchAssets = reactExports.useCallback(async () => {
        try {
            ...
            const description = data.custom_object_record.custom_object_fields?.["standard::description"];
...
function ItemRequestForm({ requestFields, serviceCatalogItem, baseLocale, hasAtMentions, userRole, userId, brandId, defaultOrganizationId, handleChange, onSubmit, attachmentsOption, attachmentsRequiredError, setAttachmentsRequiredError, isRequestFieldsLoading, isLoadingAttachmentsOption, }) {
    const { t } = useTranslation();
    ...
    const buildLookupFieldOptions = async (records, field) => {
        ...
        if (isAssetField(field)) {
            ...
            if (assetState.ids?.length) {
                list = list.filter((o) => assetState.ids.includes(o.item_asset_type_id ?? ""));
            }
            return list;
        }
    ...
    const renderRequestFields = () => {
        ...
        requestFields.forEach((field, index) => {
            ...
            if (isAssetTypeField$1(field) && isHiddenAssetsType) {
                elements.push(jsxRuntimeExports.jsxs(reactExports.Fragment, { children: [jsxRuntimeExports.jsx("input", { type: "hidden", name: field.name, value: hiddenValue }), jsxRuntimeExports.jsx("input", { type: "hidden", name: "isAssetTypeHidden", value: "true" })] }, field.id));
                return;
            }
            const customField = {
                ...field,
...
function ServiceCatalogItem({ serviceCatalogItemId, baseLocale, hasAtMentions, userRole, organizations, userId, brandId, helpCenterPath, }) {
    ...
    const { requestFields, associatedLookupField, error, setRequestFields, handleChange, isRequestFieldsLoading, } = useItemFormFields(serviceCatalogItem, baseLocale);
    const { t } = useTranslation();
    ...
    function validateAttachmentsRequired(attachments) {
        if (attachmentsOption) {
            if (attachmentsOption.custom_object_fields["standard::is_required"] &&
                attachments.length === 0) {
                setAttachmentsRequiredError(t("service-catalog.attachments-required-error", "Upload a file to continue."));
                return true;
            }
        }
